...
# \## `fetch\_strain(detector, t\_center, half\_window, cache\_dir=CACHE\_DIR)`
...
# \## `bandpass(strain, f\_low=30.0, f\_high=400.0)`
...
# \## `estimate\_psd(detector, t\_end\_event, segment\_duration=512, cache\_dir=CACHE\_DIR)`
...
# \## `extract\_frequency\_track(qtransform, t\_center, half\_window=0.25, snr\_threshold=15.0, f\_min\_search=55.0, continuity\_tol=15.0)`
...
# \## `pick\_point(t\_arr, f\_arr, t\_target, window=0.005)`
...
# \## `chirp\_mass\_from\_dfdt(dfdt, f\_ref)`
...
# \- `raw\_strain.png` — raw strain for both detectors

# \- `whitened\_strain.png` — whitened and bandpassed strain

# \- `qtransform.png` — spectrograms showing the chirp

# \- `psd.png` — detector noise ASD estimated with Welch’s method

# \- `freq\_track.png` — extracted chirp track with 0PN overlay

# \- `chirp\_mass.png` — chirp mass comparison with published value

# \- `bootstrap.png` — bootstrap distribution of chirp mass

# \- `matched\_filter\_snr.png` — matched-filter SNR time series
...
# \- \*\*Chirp mass\*\* close to the published value of \*\*28.3 \\(M\_\\odot\\)\*\*
